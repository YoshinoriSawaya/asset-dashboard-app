...
**Driveへの実書き込みは未検証。** inboxが空(E01-07で全部processedへ
移動済み)なので、取り込みが走らず書き込みの機会が無い。
検証するにはinboxにファイルを戻し、`ingested_file`テーブルを空にする
必要がある。E01-11の後にまとめて確認する。

## ステータス
完了(実書き込みの確認は残り) (2026-09-25)
